...
| `montbell` | [モンベル webshop](https://webshop.montbell.jp/goods/disp.php?product_id=1101606) | ライトアルパインダウン パーカ Men's **BK×XL** の再入荷 | 復活のたび | **1日1回**（08:05 JST） | `NTFY_TOPIC_MONTBELL` |

### 通知先を分ける（ntfyトピック）

スマホ側で「iPhoneの通知は残したいがモンベルは静かにしたい」を実現するため、**ターゲットごとに別のntfyトピックへ送れます**。ntfy アプリではトピック＝購読単位なので、片方だけミュート・通知音変更・削除ができます。

- 各 Watcher クラスの `notify_channel`（例 `"MONTBELL"`）が環境変数名になります → `NTFY_TOPIC_MONTBELL`
- **未設定なら共通の `NTFY_TOPIC` にフォールバック**するので、分けたいものだけ設定すればOK
- 分けたいターゲットの数だけ GitHub Secret を足し、`watch.yml` の `env:` に1行追加します

```bash
gh secret set NTFY_TOPIC_MONTBELL --body "montbell-restock-<推測されにくい文字列>"
```
> 先にスマホの ntfy アプリでそのトピックを購読しておくこと。購読前に設定すると、通知は誰も見ていないトピックへ飛びます。
...
- **cron ごとに対象を変えている。** `watch.yml` は `github.event.schedule`（発火した cron 式そのものが入る）で分岐し、10分ごとの回は `apple-16e`、1日1回の回は `montbell` だけを実行する。cron 式を書き換えたら `run:` 内の比較文字列も直すこと（**ここがずれると montbell が一生実行されない**）。
